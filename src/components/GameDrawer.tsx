...
export interface DrawerGame {
  ...
  items: DrawerItem[];
  /** vrai si le jeu vient du catalogue et n'est pas dans la base */
  catalogOnly?: boolean;
}
        ...
        >
          ...
          >
            ...
            <div className="space-y-4 px-5 pb-6">
                ...
                <div className="space-y-2">
                    ...
                    >
                      ...
                      <dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
                        ...
                        <dd className="text-right">
                          {COMPLETENESS_LABELS[i.completeness ?? "unknown"]}
                        </dd>
                        {i.acquiredAt ? (
                          <>
                  ...
                  {active.length === 0 ? (
                    <p className="rounded-xl border border-dashed border-border px-3 py-2.5 text-sm text-muted">
                      {row.catalogOnly
                        ? "Pas dans la collection ni la wishlist."
                        : "Aucun exemplaire en stock."}
                    </p>
                  ) : null}
                  {archived.length ? (
              ...
              {/* cotes — section toujours présente, « — » quand la cote manque */}
              <section>
                <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted">
                  ...
                  {(
                    ...
                    >
                      ...
                      <span className="font-mono text-xs">
                        {v ? (
                          <>
                            {euro(v.low)} · <span className="text-accent">{euro(v.median)}</span> ·{" "}
                            {euro(v.high)}
                          </>
                        ) : (
                ...
                {q?.cib || q?.loose ? (
                  ...
                  <p className="mt-2 text-xs text-muted">
                    Pas encore de cote pour ce jeu — l&apos;estimation se débloque dès qu&apos;une
                    cote est enregistrée.
                  </p>
                )}
              </section>

              {/* commandes liées — section toujours présente */}
              <section>
                <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted">
                  Commandes
                </h3>
              ...
              {row.catalogOnly ? (
                <p className="rounded-xl border border-border bg-surface px-4 py-2.5 text-center text-xs text-muted">
                  Pas dans la collection ni la wishlist — l&apos;ajout passe par la CLI de
                  l&apos;agent.
                </p>
              ) : (
                <Link
